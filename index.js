//Hargai Pencipta Sob! Jangan Nyolong Case
//Modal Dikit Sob!
//Gak Sopan Asal Nyolong!
//Tetap Cantumkan NamaNama Creator Bot!
...
ban = []
cr = '*46 BOT  BOT*'
vz = '*Terima Kasih Sudah Daftar Di 46  BOT*'
const vcard = 'BEGIN:VCARD\n' //
            + 'VERSION:3.0\n' //
            + 'FN: Riyan (REAL ACCOUNT)\n' //
            + 'ORG:Creator 46 BOT BOT;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=6282194303167:+6282194303167 \n' //
            + 'END:VCARD'
prefix = '#'
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Alhamdulillah Beban Grup Hilang 1 @${num.split('@')[0]} ,Jangan Balik Lagi Ya! Kalau BALIK Auto KICK! `
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: 'Maaf Ini Fitur Khusus User Mods 46 BOT  Yeager ❌',
					...
					userB: `──「 DAFTAR 」──\n*maaf sayang! Daftar Dulu Yuk, Soalnya sayang Belum Terdaftar Di Dalam Database 46 BOT*\n*Cara Daftarnya Gini yank:*\n\nKetik➷\n*/daftar |nama lengkap|umur|hobi*\n\ncontoh➷\n*/daftar |46 BOT  Yeager|17|beatbox*\n──「 46 BOT  BOT 」──`,
			...
			const ownerNumber = ["6282194303167@s.whatsapp.net","6282194303167@s.whatsapp.net","6289629713703@s.whatsapp.net"]
			const mod = [ownerNumber,"6282194303167@s.whatsapp.net","6282194303167@s.whatsapp.net","6289629713703@s.whatsapp.net"]
			const adminbotnumber = ["6282194303167@s.whatsapp.net","6282194303167@s.whatsapp.net","6289629713703@s.whatsapp.net"]
			const frendsowner = ["6282194303167@s.whatsapp.net","6282194303167@s.whatsapp.net","6289629713703@s.whatsapp.net"]
			const premium = ["6282194303167@s.whatsapp.net","6282194303167@s.whatsapp.net","6289629713703@s.whatsapp.net"]
       ...
       //function leveling
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					case 'nobg':
                                        ...
                                        anu1 = await fetchJson(`https://api.vhtear.com/removebgwithurl?link=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT 46 BOT  ✨* 」\n*│+wa.me/6281368646 BOT 011*╰──────*「 *46 BOT * 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  46 BOT  BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281368646 BOT 011*╰──────*「 *46 BOT * 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  46 BOT  BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
            ...
            break
				   ...
				   if (args.length < 1) return reply('Apa yang mau dicari kak?')
					goo = body.slice(7)
					anu = await fetchJson(`https://api.vhtear.com/googleimg?query=${goo}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(mess.wait)
				   ...
				   if (!isUser) return reply(mess.only.userB)
                    ...
                    tod = await getBuffer(`https://i.ibb.co/TmVC0Nh/1612583968963-compress47.jpg`)
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                case 'fakereplay':
                   client2.fakeReply("ange mas", "mending lari", "0821-9430-3167", MessageType.text)
                   break
				case 'infogc':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar |nama lengkap|umur|hobi\nContoh : ${prefix}/daftar |Anak haram|17|beatbox`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
					var almate = reg.split("|")[2];
						user.push(sender)
 ...
 if (!isOwner) return reply(mess.only.ownerB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufasfer = await getBuffer(`https://api.vhtear.com/galaxytext?text=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					teks = body.slice(6)
					reply(mess.wait)
					loog = await getBuffer(`https://api.vhtear.com/gamelogo?text=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${gbl11}&pasangan=${gbl21}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/harijadian?tgl=${gbl16}&bln=${gbl26}&thn=${gbl36}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(anu.result.hasil)
					break
                case 'tahta':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferrty = await getBuffer(`https://api.vhtear.com/hartatahta?text=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pinte}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${love}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/metal_maker?text=${love}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
				case 'apiteks':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/fire_maker?text=${love}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
				case 'ffbaner':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/bannerff?title=${love}&text=ARIFI%20RAZZAQ&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
					case 'goldbutton':
				if (isBanned) return reply(mess.only.benned)    
          ...
          if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferw = await getBuffer(`https://api.vhtear.com/thundertext?text=${thun}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
                      ...
                      gl1 = gdh.split("|")[0];
                      gl2 = gdh.split("|")[1];
                      buffere = await getBuffer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
                      reply(mess.wait)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					part = body.slice(7)
					reply(mess.wait)
					bufferu = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					part = body.slice(7)
					reply(mess.wait)
					bufferu = await getBuffer(`https://api.vhtear.com/logoml?hero=miya&text=${part}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					client.sendMessage(from, bufferu, image, {caption: 'Nih kak', quoted: mek})
					break
				case 'shadow':
				if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferi = await getBuffer(`https://api.vhtear.com/romancetext?text=${tels5}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6289629713703@s.whatsapp.net', options, text, {quoted: mek})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					teksj = body.slice(7)
					reply(mess.wait)
					anwu = await fetchJson(`https://api.vhtear.com/walpaper?query=${teksj}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
						...
						res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/nhentaipdfdownload?query=287167&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomcat?apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/mlherolist?apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
 ...
 if (isBanned) return reply(mess.only.benned)    
          ...
          reply('Wait..')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('teks nya mana om')
					kj = body.slice(12)
					anu = await fetchJson(`https://api.vhtear.com/nomerhoki?no=${kj}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(anu.result.hasil)
					break
				case 'textscreen':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('teks nya mana om')
					telsjk = body.slice(9)
					anu = await fetchJson(`https://api.vhtear.com/textscreen?query=${telsjk}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (data.error) return reply(data.error)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewanimutlucu&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/tebakgambar&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/funkuis&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					setTimeout( () => {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/family100&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					setTimeout( () => {
					client.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/branly?query=${body.slice(9)}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=elang&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=naruto&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=minato&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=boruto&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hinata&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=sasuke&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=sakura&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=unta&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=kaneki&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewanimut&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var kan = JSON.parse(JSON.stringify(anu.result));
					var eki =  kan[Math.floor(Math.random() * kan.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=ToukaKirishima&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=RizeKamishiro&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=akiramado&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=itori&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=kurumitokisakikawai&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=Nakanomiku&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=remrezero&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=ramrezero&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=bokep&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cewekcantik&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cowokganteng&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					setTimeout( () => {
					 client.sendMessage(from, 'siap siap ya 46 BOT  Kick nih', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 10s,
					}
					break
				case 'aguse':
				...
				if (!isUser) return reply(mess.only.userB)
					if (args.length < 1) return reply(mess.search)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewan&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(mess.wait)
					var pin = JSON.parse(JSON.stringify(anu.result));
					var trest =  pin[Math.floor(Math.random() * pin.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=lolikawai&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=waifukawai&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=nekoanimekawai&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isNsfw) return reply('❌ *FALSE* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animehentai&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=loli&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply(mess.search)
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pin}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					reply(mess.wait)
					var pin = JSON.parse(JSON.stringify(anu.result));
					var trest =  pin[Math.floor(Math.random() * pin.length)];
					pine = await getBuffer(trest)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/tiktokdl?link=https://vt.tiktok.com/yqyjPX/&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					if (anu.error) return reply(anu.error)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					tulis = body.slice(6)
					reply(mess.wait)
					buffer4 = await getBuffer(`https://api.vhtear.com/write?text=${tulis}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			     case 'airtext':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/water_maker?text=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			     case 'kuncitext':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/padlock?text1=${teks}&text2=${teks}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
                    client.sendMessage(from, buffer6, image, {quoted: mek, caption: `${teks}`})
			     	break
			    case 'shorturl':
			if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${ige}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomobil?merk=${ige}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					ft1 = body.slice(11)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomotor?merk=${ft1}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					kuji = body.slice(7)
					reply(mess.wait)
					anu = await getBuffer(`https://api.vhtear.com/playstore?query={kuji}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/url2image?tipe=${args[0]}&url=${args[1]}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/logoff?hero=alok&text=${tels}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
					buffer = await getBuffer(anu.result)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  46 BOT  BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner 46 BOT  !', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin 46 BOT  !')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
